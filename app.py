...
APP_VERSION = "v9.8.3-solution-boundary"
...
def phase_reply_issues(answer, action, history):
    ...
    if action == "explain_solution" and "?" in answer:
        interest_question = bool(re.search(r"(хотите|интересно|имеет ли смысл|подош[её]л бы|полезно ли).{0,80}(увидеть|посмотреть|узнать|решени|помощник|ассистент|подход)", low))
        domain_question = bool(re.search(r"(как(ие|ой|ую)?|что|где|когда|сколько).{0,100}(игр|жанр|механик|тем|курс|урок|упражнен|материал|структур|персонаж|контент)", low))
        if domain_question and not interest_question:
            issues.append("проектирование результата клиента вместо объяснения ИИ-решения")
    if action == "explain_solution" and re.search(r"(давайте.{0,40}(разбер|определ|выбер|подума|найд)|начн[её]м с того.{0,40}(определ|выясн|выбер))", low):
        issues.append("попытка начать рабочую консультацию внутри чата")
...
Коротко откликнитесь на одну конкретную деталь клиента и задайте один понятный вопрос. Не угадывайте его задачу по профессии, не предлагайте категории и варианты ответа. Если клиент пока сообщил только профессию и аудиторию, спросите о его реальной рабочей трудности или причине обращения к эксперту, не сужая тему до материалов, контента, продаж или других предполагаемых областей. Не пересказывайте ответ и не обсуждайте решение, продукт или встречу."""
    if action == "explain_solution":
        return """Диагностика закончена. Больше ничего не выясняйте. Коротко назовите выявленный разрыв и объясните одно конкретное направление ИИ-решения из базы знаний: какой тип помощника или системы снимает проблему и что он сможет удерживать или делать. Не разрабатывайте сам результат клиента и не спрашивайте о его содержании — темах, играх, жанрах, механиках, структуре, материалах, упражнениях или персонажах. В конце допустим только один вопрос: интересно ли клиенту увидеть предложенное ИИ-решение на своём примере. Не приглашайте на консультацию."""
...
CONSULTING_IN_CHAT = re.compile(
    r"(обсудим|подумаем|разбер[её]м|давайте.{0,25}(подбер|придума|состав)).{0,100}"
    r"(тем|формат|иде|структур|курс|урок|контент|стратег|упражнен)",
    re.I,
)
...
def quality_issues(answer):
    ...
    if CONSULTING_IN_CHAT.search(answer):
        issues.append("попытка консультировать клиента внутри чата вместо выявления потребности")
    return issues
...
Не начинайте решать профессиональную задачу клиента и не предлагайте вместе придумывать темы, структуру, курс, урок, стратегию или контент.
{profile.get('style_rules', '')}
