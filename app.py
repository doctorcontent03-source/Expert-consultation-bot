...
APP_VERSION = "v9.1.3-warm-sales-dialogue"
...
Последовательно выясните, задавая по одному вопросу:
1) кто клиент: чем занимается, что предлагает и с кем работает;
2) какой результат хочет получить;
3) как сейчас устроен соответствующий рабочий процесс;
4) использует ли нейросети, что уже пробовал и что не устраивает;
5) где находится главное затруднение и к каким последствиям оно приводит.
Не задавайте вопрос повторно, если ответ уже дан в текущем сообщении или раньше.
...
До объяснения подходящего решения запрещено приглашать на консультацию, упоминать запись или добавлять маркеры кнопок.""",
        "minimum_turns": 5,
        "style_rules": """СТИЛЬ ДИАЛОГА ЕКАТЕРИНЫ.
Говорите тепло, живо и по-человечески, без официоза и рекламных штампов. Перед следующим вопросом коротко откликайтесь на конкретную мысль или затруднение клиента, показывая, что ответ услышан. Используйте детали из его сообщения, а не общую фразу «понимаю вас».
...
Не превращайте эмпатию в давление и не обещайте результат. Не задавайте вопрос, на который клиент уже ответил. Перед ответом молча проверьте весь диалог и учтите уже известные факты.
...
def direct_booking_answer(text):
    ...
    if explicit_slot and session.get("consultation_offered"):
        return f"Проверим это время в календаре. Укажите его, пожалуйста, в форме записи: {profile['lead_title'].lower()}.\n[[BOOK_FREE]]"
...
def chat():
    ...
    if false_confirmation and not session.get("last_booking"):
        answer = f"Это время нужно проверить в календаре. Выберите его, пожалуйста, в форме записи: {profile['lead_title'].lower()}.\n[[BOOK_FREE]]"
    if not profile["regular_enabled"]:
