...
APP_VERSION = "v8.3-contact-and-format"
...
Задавайте преимущественно один вопрос за раз. Не предлагайте консультацию после первой общей реплики клиента. Сначала выясните как минимум длительность ситуации, её влияние на жизнь и что человек хотел бы изменить. До этого только поддерживайте разговор и задавайте уточняющие вопросы. После первого предложения не повторяйте его, пока клиент сам явно не согласится записаться. Если клиент просит не торопить его, хочет сначала получить информацию, сомневается или задаёт вопрос об условиях, отвечайте только на вопрос и не завершайте ответ новым предложением консультации.
...
Не придумывайте очный приём, города, адреса, платформы связи или дополнительные форматы работы. Основной формат работы указан ниже в настройках. Называйте его прямо, без слов «тоже», «также», «если вам удобнее» и без намёка, что другой формат является основным.
...
def direct_format_answer(text):
    work_format = os.getenv("EXPERT_WORK_FORMAT", "").strip()
    if not work_format:
        return None
    low = text.lower()
    if re.fullmatch(r".{0,20}(онлайн|очно).{0,20}", low) or re.search(r"(вы|консультац|работ).{0,20}(онлайн|очно)", low):
        return f"Я работаю {work_format}."
    return None
...
def chat():
    ...
    format_answer = direct_format_answer(text)
    if format_answer:
        con.execute("insert into messages values(?,?,?,?)",(sid,"assistant",format_answer,int(time.time()*1000))); con.commit()
        return jsonify(answer=format_answer)
    ...
    work_format = os.getenv("EXPERT_WORK_FORMAT", "").strip() or "используйте только формат, прямо указанный в базе знаний"
    booking_rules = f"\n\nНАСТРОЙКИ:\nОсновной формат работы: {work_format}.\nБесплатная встреча: {free_title}, {free_duration} минут. Регулярная встреча: {regular_title}, {regular_duration} минут."
